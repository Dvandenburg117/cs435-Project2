class weightedGraph(Graph):
    ...
    def addWeightedEdge(self,first,second,edgeWeight):
        if first.id in self.vertices and second.id in self.vertices:
            ...
            self.insertIntoAdj(self, first.id, lst1)
    ...
    def createRandomCompleteWeightedGraph(self,n):
        ...
        for ii in range(index):
            node1 = Vertex(self, random.randint(1,n))
            randomNode = random.random(0, n)
            node2 = Vertex.node(self, randomNode)
            self.addWeightedEdge(self,node1,node2,random.randint(1,index))
    def createLinkedList(self, n):
        for i in range(n):
            self.addNode(self,i)
        for i in range(1,n):
            node1 = Vertex.node(i - 1)
            node2 = Vertex.node(i)
        ...
        while len(self.vertices) != len(visited):
            ...
            for node in self.getwNeighbors(curr):
                if node.id not in visited:
                    ...
                    curr = self.Vertex.node(nextNode)
        return retDic
